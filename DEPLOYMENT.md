...
9. [Видео-трейлеры в галерее](#9-видео-трейлеры-в-галерее)
...
# ffmpeg — нужен для превью-трейлеров видео в галерее (см. §9).
# Без него /api/video-preview просто отключается (отдаёт 501),
...
галерея показывает короткий автовоспроизводящийся «трейлер» — как превью
на видеохостингах — вместо статичной картинки.

### 9.1 Как это работает
- Трейлер генерируется по требованию: несколько кадров (10-30, в зависимости
  от длительности видео) вырезаются точечными seek'ами (без декодирования
  всего видео) и собираются в один маленький анимированный WebP через ffmpeg.
- Результат кэшируется точно так же, как обычные превью фото: сначала
  локальный диск (`.data/`), затем рядом с исходником — `.previews/` в папке
  на WebDAV (или в `public/.../.previews/` в демо-режиме). Повторные показы
  того же видео не требуют повторной генерации.
- Одновременно на сервере выполняется только **одна** задача ffmpeg — это
  специально сделано, чтобы всплеск наведений в галерее не перегрузил
  однoядерный VPS.
- Готовый трейлер обычно весит 20-60 КБ — это не видеофайл, а маленькая
  анимированная картинка, так что дисковый бюджет почти не страдает даже
  при большом количестве видео.
- Если у видео нет пары (например, само видео без миниатюры) или ffmpeg не
  установлен, галерея просто показывает обычную статичную превьюшку —
  никаких ошибок для пользователя.

### 9.2 Требования
- **ffmpeg** должен быть установлен (см. §1.2: `sudo apt install -y ffmpeg`).
  Без него `/api/video-preview` отвечает `501` и превью-трейлеры тихо
  отключаются — остальная галерея не затрагивается.

### 9.3 Настройка (необязательно)
Переменные окружения — можно не задавать, есть разумные значения по умолчанию:

| Переменная | По умолчанию | Назначение |
|---|---|---|
| `VIDEO_PREVIEW_MAX_MB` | `300` | Видео больше этого размера не обрабатываются (защита диска от случайно огромного файла) |
| `COLOCATED_PREVIEWS_DIR` | `.previews` | Имя папки рядом с видео, где хранится сгенерированный трейлер |
| `WEBDAV_COLOCATED_CACHE` | `true` | Если `false` — трейлеры не сохраняются обратно на WebDAV, только в локальном кэше сервера |

### 9.4 Очистка «осиротевших» превью
При запуске полной (по всей галерее, а не по одной папке) генерации превью через `/api/images/generate`
(см. кнопку в интерфейсе) заодно удаляются и превью-трейлеры для видео,
которые с тех пор были удалены или переименованы — так же, как это уже
происходит для обычных фото-миниатюр.
