...
### 7. Быстрая навигация по папкам (dir-meta covers)
**Проблема:** При возврате из папки с большим количеством фото (~800) в каталог папок загрузка занимала ~20 секунд из-за N+1 запросов PROPFIND к WebDAV для получения превью каждой подпапки.

**Решение:**
- Пути обложек хранятся локально в `dir-meta.json` (поле `covers`) — чтение с диска вместо WebDAV
- При первом входе в папку автоматически сохраняются первые 3 фото как обложки
- Пользователь может вручную выбрать обложки (до 3 шт.) через кнопку ⭐
- `/api/folders` читает обложки из локальных файлов — мгновенная загрузка

**Преимущества:**
- ✅ Загрузка каталога папок: ~20 сек → < 1 сек
- ✅ Нулевые дополнительные запросы к WebDAV при отображении папок
